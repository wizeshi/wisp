...
/** WispYtdlpManagerPlugin */
class WispYtdlpManagerPlugin :
    FlutterPlugin,
    MethodCallHandler {
    ...
    private fun resolveNativeLibraryRuntime(assetAbi: String?): NodeRuntime? {
        val nativeDirPath = context.applicationInfo?.nativeLibraryDir ?: return null
    ...
    private fun ensureExtractedAssetLibDir(assetAbi: String): File? {
        ...
        val abiAssets = context.assets.list(abiAssetRoot) ?: return null
        ...
        val assetLibFiles = context.assets.list(assetLibPath) ?: emptyArray()

        if (!hasZipPayload && assetLibFiles.isEmpty()) {
            return null
        }

        val outputLibDir = File(context.filesDir, "node-runtime/$assetAbi/lib")
    ...
    private fun resolveAssetAbi(): String? {
        val availableAbis = context.assets.list(NODE_ASSET_ROOT)?.toSet() ?: emptySet()
