...
/// The subtitle shown under a playlist's title: the Spotify-provided
/// description when Spotify itself is the author (their descriptions are
/// usually more informative than "Spotify"), otherwise the author's name,
/// falling back to the description if there's no author at all.
///
/// Pulled out as a standalone function (rather than private to the card)
/// so anywhere else that needs "the subtitle line for a playlist" — a row,
/// a detail header — gets the same rule instead of a slightly different
/// reimplementation.
...
/// A [GenericCard] for a [GenericPlaylist].
class PlaylistCard extends StatelessWidget {
  ...
  Future<void> _togglePlaylistPlayback(BuildContext context) async {
    ...
    if (audioHandler != null) {
      ...
      } else { // Playlist is not active, so play it.
