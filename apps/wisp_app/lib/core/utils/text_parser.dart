...
class TextParser {
  ...
  static bool needsParsing(String input) {
    return (_anchorTagRegex.hasMatch(input) ||
        _htmlEntityRegex.hasMatch(input));
  }
  ...
  static List<InlineSpan> parse(
    ...
    for (final match in _anchorTagRegex.allMatches(input)) {
      ...
      if (spotifyLink != null &&
          _supportedSpotifyTypes.contains(spotifyLink.type)) {
        spans.add(
          WidgetSpan(
  ...
  static Widget build(
    ...
    return Text.rich(
      TextSpan(
        style: baseStyle,
        children: parse(context, input, style: baseStyle, linkStyle: linkStyle),
      ),
      textAlign: textAlign,
      textDirection: textDirection,
      softWrap: softWrap,
...
List<InlineSpan> parseText(
  ...
  return TextParser.parse(context, input, style: style, linkStyle: linkStyle);
}
